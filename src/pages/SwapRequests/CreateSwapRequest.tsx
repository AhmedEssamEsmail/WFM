  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { error: insertError } = await supabase.from('swap_requests').insert({
        ...
        requester_original_shift_type_on_target_date: myShifts.find(
          (s) => s.date === targetShifts.find((t) => t.id === targetShiftId)?.date
        )?.shift_type,
        target_original_shift_type_on_requester_date: targetShifts.find(
    ...
    <div className="mx-auto max-w-2xl">
      <div className="rounded-lg bg-white shadow">
        ...
        <form onSubmit={handleSubmit} className="space-y-6 p-6">
          ...
          {canSubmitOnBehalf && (
            <div>
              ...
              <p className="mt-1 text-sm text-gray-500">
                As a {user?.role?.toUpperCase()}, you can submit swap requests on behalf of any
                user.
              </p>
            </div>
          )}
          ...
          <div>
            <label className="mb-2 block text-sm font-medium text-gray-700">Swap With Agent</label>
            <select
              value={targetUserId}
            ...
            {loadingAgents && <p className="mt-1 text-sm text-gray-500">Loading agents...</p>}
          </div>
