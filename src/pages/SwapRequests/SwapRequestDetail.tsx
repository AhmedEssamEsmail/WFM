...
export default function SwapRequestDetail() {
  ...
  async function handleApprove() {
    ...
    try {
      ...
      const updatedRequest = await swapRequestsService.updateSwapRequestStatus(
        id!,
        newStatus,
        approvalField,
        oldStatus
    ...
    try {
      ...
      if (
        ...
        if (reqOnReqDate && request.requester_original_shift_type) {
          await shiftsService.updateShift(reqOnReqDate.id, {
            shift_type: request.requester_original_shift_type,
          });
        }

        if (reqOnTgtDate && request.requester_original_shift_type_on_target_date) {
          await shiftsService.updateShift(reqOnTgtDate.id, {
            shift_type: request.requester_original_shift_type_on_target_date,
          });
        }

        if (tgtOnReqDate && request.target_original_shift_type_on_requester_date) {
          await shiftsService.updateShift(tgtOnReqDate.id, {
            shift_type: request.target_original_shift_type_on_requester_date,
          });
        }

        if (tgtOnTgtDate && request.target_original_shift_type) {
          await shiftsService.updateShift(tgtOnTgtDate.id, {
            shift_type: request.target_original_shift_type,
          });
        }
      }
        ...
        <div className="rounded-lg bg-white p-6 shadow-sm">
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
            <div className="rounded-lg border p-4">
              ...
              <div className="mb-1 h-5 w-28 animate-pulse rounded bg-gray-200"></div>
              <div className="mb-3 h-8 w-24 animate-pulse rounded bg-gray-200"></div>
              <div className="mb-1 h-5 w-28 animate-pulse rounded bg-gray-200"></div>
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Swap Details */}
      <div className="rounded-lg bg-white p-6 shadow-sm">
        <h2 className="mb-4 text-lg font-semibold text-gray-900">Swap Details</h2>
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
          {/* Requester's Shifts */}
          <div className="rounded-lg border p-4">
            <h3 className="mb-2 font-medium text-gray-900">Requester's Shifts</h3>
            <p className="mb-3 text-sm text-gray-500">{requester.name}</p>

            {/* Date 1: requester_original_date */}
            {request.requester_original_date && (
              <div className="mb-3">
                <p className="font-medium">{formatDate(request.requester_original_date)}</p>
                <span className="mt-1 inline-block rounded bg-blue-100 px-2 py-1 text-sm text-blue-800">
                  {request.requester_original_shift_type
                    ? getShiftDisplay(request.requester_original_shift_type as ShiftType).name
                    : 'Unknown'}
                </span>
              </div>
            )}
            ...
            {request.target_original_date &&
              request.target_original_date !== request.requester_original_date && (
                <div>
                  <p className="font-medium">{formatDate(request.target_original_date)}</p>
                  <span className="mt-1 inline-block rounded bg-blue-100 px-2 py-1 text-sm text-blue-800">
                    {request.requester_original_shift_type_on_target_date
                      ? getShiftDisplay(
                          request.requester_original_shift_type_on_target_date as ShiftType
                        ).name
                      : 'Unknown'}
                  </span>
                </div>
          ...
          {/* Target's Shifts */}
          <div className="rounded-lg border p-4">
            <h3 className="mb-2 font-medium text-gray-900">Target's Shifts</h3>
            <p className="mb-3 text-sm text-gray-500">{targetUser.name}</p>

            {/* Date 1: requester_original_date */}
            {request.requester_original_date && (
              <div className="mb-3">
                <p className="font-medium">{formatDate(request.requester_original_date)}</p>
                <span className="mt-1 inline-block rounded bg-purple-100 px-2 py-1 text-sm text-purple-800">
                  {request.target_original_shift_type_on_requester_date
                    ? getShiftDisplay(
                        request.target_original_shift_type_on_requester_date as ShiftType
                      ).name
                    : 'Unknown'}
                </span>
              </div>
                ...
                <div>
                  ...
                  <span className="mt-1 inline-block rounded bg-purple-100 px-2 py-1 text-sm text-purple-800">
                    {request.target_original_shift_type
                      ? getShiftDisplay(request.target_original_shift_type as ShiftType).name
                      : 'Unknown'}
                  </span>
                </div>
              )}
      ...
      {/* Approval Timeline */}
      <div className="rounded-lg bg-white p-6 shadow-sm">
        <h2 className="mb-4 text-lg font-semibold text-gray-900">Approval Timeline</h2>
        <div className="space-y-4">
          {/* Created Step */}
          ...
          <div className="flex items-center gap-3">
            ...
            >
              {request.status === 'rejected' &&
              (request.tl_approved_at || !request.wfm_approved_at) ? (
                <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
