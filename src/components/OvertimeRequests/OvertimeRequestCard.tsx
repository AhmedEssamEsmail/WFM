  ...
  return (
    ...
    >
      ...
      <div className="mb-3 flex items-start justify-between">
        ...
        {/* Overtime Type Badge */}
        <span
          className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-medium ${getOvertimeTypeColor(request.overtime_type)}`}
      ...
      {/* Reason (Truncated) */}
      <div className="mb-3">
        <p className="line-clamp-2 text-sm text-gray-600">{truncateReason(request.reason)}</p>
      </div>
      ...
      <div className="flex items-center justify-between border-t border-gray-100 pt-3">
        ...
        {/* Approval Info */}
        {approvalInfo && <span className="text-xs text-gray-500">{approvalInfo}</span>}
      </div>
    </div>
  );
}
