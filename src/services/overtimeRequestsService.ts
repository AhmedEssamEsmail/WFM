...
export const overtimeRequestsService = {
  ...
  async createOvertimeRequest(input: CreateOvertimeRequestInput): Promise<OvertimeRequest> {
    ...
    if (!input.reason || input.reason.length < 10 || input.reason.length > 250) {
      throw new ValidationError(
        'reason',
        input.reason,
        'Reason must be between 10 and 250 characters'
      );
    }
   ...
   */
  async getOvertimeRequests(filters: OvertimeRequestFilters = {}): Promise<{
    ...
    if (filters.department) {
      filteredData = filteredData.filter((req) => req.requester?.department === filters.department);
    }
  ...
  async approveOvertimeRequest(id: string, notes: string): Promise<OvertimeRequest> {
    ...
    if (currentRequest.status === 'pending_tl') {
      ...
      throw new ValidationError(
        ...
        'Request cannot be approved in current status'
      );
    }
    ...
    await commentsService.createSystemComment(id, 'overtime_request', commentContent, user.id);
  ...
  async getOvertimeStatistics(filters: OvertimeRequestFilters = {}): Promise<OvertimeStatistics> {
    ...
    const pending = requests.filter(
      (r) => r.status === 'pending_tl' || r.status === 'pending_wfm'
    ).length;
