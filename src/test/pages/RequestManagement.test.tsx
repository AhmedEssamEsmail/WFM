...
describe('RequestManagement Page', () => {
  ...
  const mockLeaveRequests = [
    {
      id: 'leave-1',
      user_id: 'user-5',
      user: { id: 'user-5', name: 'Charlie Davis', email: 'charlie@example.com', role: 'agent' as const },
      leave_type: 'vacation',
    ...
    },
    {
      id: 'leave-2',
      user_id: 'user-6',
      user: { id: 'user-6', name: 'Diana Evans', email: 'diana@example.com', role: 'agent' as const },
      leave_type: 'sick',
  ...
  beforeEach(() => {
    ...
    vi.mocked(useAuthModule.useAuth).mockReturnValue({
      user: { id: 'tl-1', name: 'TL User', email: 'tl@example.com', role: 'tl', created_at: '2024-01-01' },
      signOut: vi.fn(),
      isLoading: false,
    })
  ...
  it('renders the page title and description', () => {
    ...
    expect(screen.getByText('View and manage all swap and leave requests in one place')).toBeInTheDocument()
  })
  ...
  it('navigates to leave request detail page when row is clicked', async () => {
    ...
    const table = document.querySelector('table')!
    const rows = table.querySelectorAll('tbody tr')
    const charlieDavisRow = Array.from(rows).find(row => row.textContent?.includes('Charlie Davis'))
  ...
  it('displays approve and reject buttons for TL on pending requests', async () => {
    ...
    const actionsCell = within(johnDoeRow as HTMLElement).getByText('Approve').closest('td')!
