...
describe('RequestTable Component', () => {
  ...
  describe('Mobile Card View', () => {
    it('should render cards for mobile view', () => {
      ...
      const cards = screen.getAllByRole('button').filter(el => 
        el.getAttribute('aria-label')?.includes('View')
      )
    ...
    it('should call onRowClick when mobile card is clicked', () => {
      ...
      const mobileCards = screen.getAllByRole('button').filter(el => 
        el.getAttribute('aria-label')?.includes('View swap request from John Doe')
      )
  ...
  describe('Accessibility', () => {
    ...
    it('should have proper ARIA labels on action buttons', () => {
      ...
      expect(screen.getAllByLabelText('Approve request from John Doe').length).toBeGreaterThanOrEqual(1)
      expect(screen.getAllByLabelText('Reject request from John Doe').length).toBeGreaterThanOrEqual(1)
      expect(screen.getAllByLabelText('Revoke request from Bob Wilson').length).toBeGreaterThanOrEqual(1)
    })
