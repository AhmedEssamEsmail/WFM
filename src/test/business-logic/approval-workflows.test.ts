...
describe('Approval Workflow Tests', () => {
  ...
  afterEach(async () => {
    ...
    testUserIds = []; testShiftIds = []; testSwapIds = []; testLeaveIds = [];
  });
  ...
  describe('Swap Request Workflow', () => {
    it('should follow pending_acceptance → pending_tl → pending_wfm → approved', async () => {
      ...
      let { data: updated } = await testSupabase.from('swap_requests').select('status').eq('id', swap!.id).single();
      expect(updated!.status).toBe('pending_tl');

      // TL approves
      await testSupabase.from('swap_requests').update({ 
        status: 'pending_wfm',
        tl_approved_at: new Date().toISOString()
      }).eq('id', swap!.id);
      ({ data: updated } = await testSupabase.from('swap_requests').select('status').eq('id', swap!.id).single());
    ...
    it('should allow rejection at pending_tl stage', async () => {
      ...
      const { data: rejected } = await testSupabase.from('swap_requests').select('status').eq('id', swap!.id).single();
  ...
  describe('Leave Request Workflow', () => {
    it('should follow pending_tl → pending_wfm → approved', async () => {
      ...
      let { data: updated } = await testSupabase.from('leave_requests').select('status').eq('id', leave!.id).single();
      expect(updated!.status).toBe('pending_wfm');
      ...
      ({ data: updated } = await testSupabase.from('leave_requests').select('status').eq('id', leave!.id).single());
      expect(updated!.status).toBe('approved');
    });
    ...
    it('should support auto-denial for insufficient balance', async () => {
      ...
      // Set low balance
      await testSupabase.from('leave_balances').update({ balance: 1 })
        .eq('user_id', user!.id).eq('leave_type', 'casual');
  ...
  describe('Status Transition Validation', () => {
    it('should record approval timestamps', async () => {
      ...
      const { data: final } = await testSupabase.from('leave_requests')
        .select('tl_approved_at, wfm_approved_at').eq('id', leave!.id).single();
