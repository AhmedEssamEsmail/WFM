...
describe('Comments and Exception Tests', () => {
  ...
  afterEach(async () => {
    if (testCommentIds.length) await testSupabase.from('comments').delete().in('id', testCommentIds);
    ...
    testUserIds = []; testLeaveIds = []; testCommentIds = [];
  });
  ...
  describe('Comment System', () => {
    it('should create user comments on leave requests', async () => {
      ...
      const { data: leave } = await testSupabase.from('leave_requests').insert({
        user_id: user!.id,
        leave_type: 'annual',
    ...
    it('should create system comments', async () => {
      ...
      const { data: sysComment } = await testSupabase.from('comments').insert({
        request_type: 'leave',
        request_id: leave!.id,
        user_id: user!.id,
    ...
    it('should distinguish between user and system comments', async () => {
      ...
      // Create both types
      const { data: comments } = await testSupabase.from('comments').insert([
        { request_type: 'leave', request_id: leave!.id, user_id: user!.id, content: 'User comment', is_system: false },
        { request_type: 'leave', request_id: leave!.id, user_id: user!.id, content: 'System comment', is_system: true }
      ]).select();
      testCommentIds.push(...comments!.map(c => c.id));

      const { data: userComments } = await testSupabase.from('comments')
        .select('*').eq('request_id', leave!.id).eq('is_system', false);
      const { data: systemComments } = await testSupabase.from('comments')
        .select('*').eq('request_id', leave!.id).eq('is_system', true);
  ...
  describe('Exception Request Workflow', () => {
    it('should support denied → exception request flow', async () => {
      ...
      const { data: denied } = await testSupabase.from('leave_requests').insert({
        user_id: user!.id,
        leave_type: 'annual',
      ...
      const { data: exception } = await testSupabase.from('leave_requests').insert({
        user_id: user!.id,
        leave_type: 'annual',
        start_date: '2027-01-15',
        end_date: '2027-01-20',
    ...
    it('should allow exception approval workflow', async () => {
      ...
      await testSupabase.from('leave_requests').update({ 
        status: 'pending_wfm',
      ...
      const { data: approved } = await testSupabase.from('leave_requests')
        .select('status').eq('id', exception!.id).single();
    ...
    it('should allow exception rejection', async () => {
      ...
      await testSupabase.from('leave_requests').update({ status: 'rejected' }).eq('id', exception!.id);

      const { data: rejected } = await testSupabase.from('leave_requests')
        .select('status').eq('id', exception!.id).single();
