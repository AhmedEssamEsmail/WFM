...
import { agentBreakScheduleArb, timeArb, generateIntervals } from '../generators/breakScheduleGenerators'
...
describe('Coverage and Display Properties', () => {
    ...
    fc.assert(
      fc.property(
        ...
        (date) => {
          ...
          const pattern = /^(Sunday|Monday|Tuesday|Wednesday|Thursday|Friday|Saturday), (Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec) \d{1,2}, \d{4}$/
  ...
  it('Property 19: Date navigation arithmetic', () => {
    fc.assert(
      fc.property(
        fc.date({ min: new Date('2024-01-02'), max: new Date('2026-12-30') }).filter(d => !isNaN(d.getTime())),
        (currentDate) => {
