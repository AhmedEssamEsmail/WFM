      ...
      fc.asyncProperty(
        ...
        async (agents, date) => {
          ...
          const validAgents = agents.filter(
            (a) => a.shift_type !== 'OFF' && (a.breaks.HB1 || a.breaks.B || a.breaks.HB2)
          );
          ...
          try {
            ...
            if (typeof (csvBlob as any).text === 'function') {
              ...
              const blobParts = (csvBlob as any)[Symbol.for('nodejs.util.inspect.custom')] ||
                (csvBlob as any)._parts || [(csvBlob as any).toString()];
      ...
      fc.property(
        ...
        ({ agentId, importDate, existingBreaks, newBreaks, agentInImport }) => {
          ...
          if (agentInImport) {
            ...
            expect(currentBreaks).not.toContain(
              ...existingBreaks.filter((b) => !newBreaks.includes(b))
            );
          } else {
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (rows) => {
          ...
          const hasInvalidDate = rows.some(
            (r) => typeof r.date === 'string' && !/^\d{4}-\d{2}-\d{2}$/.test(r.date)
          );
          const hasInvalidShift = rows.some(
            (r) => !['AM', 'PM', 'BET', 'OFF'].includes(r.shift as string)
          );
