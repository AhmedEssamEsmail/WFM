...
describe('Error Boundary Wrapping Properties', () => {
  ...
  describe('PageErrorBoundary catches errors', () => {
    it('Property 21: Should catch errors in wrapped components', () => {
      fc.assert(
        fc.property(fc.boolean(), errorMessageArb, (shouldThrow, message) => {
          cleanup();
          render(
          ...
          if (shouldThrow) {
            ...
            expect(
              screen.getByText(/We're sorry, but this page encountered an unexpected error/i)
            ).toBeInTheDocument();
          } else {
          ...
          (shouldThrow, customMessage) => {
            ...
            render(
              <QueryClientProvider client={queryClient}>
                <BrowserRouter>
                  <AuthContext.Provider
                    value={{ user: mockUser, loading: false, signOut: vi.fn() }}
                  >
                    <ToastProvider>
