...
const testUsers = [
  ...
  // Agent users
  {
    email: 'agent1@dabdoob.com',
    name: 'John Smith',
    role: 'agent',
    department: 'Customer Support',
  },
  {
    email: 'agent2@dabdoob.com',
    name: 'Maria Garcia',
    role: 'agent',
    department: 'Customer Support',
  },
  ...
  {
    email: 'agent5@dabdoob.com',
    name: 'Robert Taylor',
    role: 'agent',
    department: 'Technical Support',
  },
  {
    email: 'agent6@dabdoob.com',
    name: 'Amanda White',
    role: 'agent',
    department: 'Technical Support',
  },
  {
    email: 'agent7@dabdoob.com',
...
function generateShifts(userId: string, startDate: Date, days: number) {
  ...
  for (let i = 0; i < days; i++) {
    ...
    const shiftType = Math.random() < 0.8 ? shiftTypes[Math.floor(Math.random() * 3)] : 'OFF';
...
async function seedShifts(users: any[]) {
  ...
  const { data: shifts, error } = await supabase.from('shifts').insert(allShifts).select();
...
async function seedSwapRequests(users: any[], shifts: any[]) {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    const target = agents.filter((a) => a.id !== requester.id)[
      Math.floor(Math.random() * (agents.length - 1))
    ];

    const requesterShifts = shifts.filter(
      (s) => s.user_id === requester.id && s.shift_type !== 'OFF'
    );
...
async function seedComments(swapRequests: any[], leaveRequests: any[], users: any[]) {
  ...
  if (comments.length > 0) {
    const { data: commentData, error } = await supabase.from('comments').insert(comments).select();
