...
import { leaveTypeSchema, leaveRequestValidationSchema } from './schemas/leaveRequest';
import { shiftTypeSchema, swapRequestValidationSchema } from './schemas/swapRequest';
...
export function validateDateRange(
  ...
  if (!result.success) {
    ...
    throw new ValidationError(
      fieldName,
      error.path[0] === 'start_date' ? startDate : endDate,
      error.message
    );
  }
}
...
export function validateLeaveType(leaveType: string): void {
  ...
  if (!result.success) {
    throw new ValidationError('leave_type', leaveType, result.error.issues[0].message);
  }
}
...
export function validateShiftType(shiftType: string): void {
  ...
  if (!result.success) {
    throw new ValidationError('shift_type', shiftType, result.error.issues[0].message);
  }
}
 ...
 */
export function validateBreakOrdering(intervals: Record<string, string>): {
  valid: boolean;
  error?: string;
} {
